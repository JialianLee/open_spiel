...
class _InfoStateNode(object):
  ...
  current_cfr_regret = attr.ib(factory=lambda: collections.defaultdict(float))
  cumulative_cf_reach_prob = attr.ib(factory=lambda : collections.defaultdict(dict))
  cumulative_utility = attr.ib(factory=lambda: collections.defaultdict(float))
  last_cfr_regret = attr.ib(factory=lambda: collections.defaultdict(float))
  opt_util = attr.ib(factory=lambda: collections.defaultdict(float))
...
class _CFRSolverBase(object):
  ...
  def __init__(self, game, update_count, alternating_updates, linear_averaging,
  ...
  def _compute_counterfactual_regret_for_player(self, state, policies,
    ...
    for action, action_prob in info_state_policy.items():
      ...
      cf_reach_prob = info_state_node.cumulative_cf_reach_prob[action].get(state, 0)
      info_state_node.cumulative_cf_reach_prob[action][state] = cf_reach_prob + counterfactual_reach_prob
      cur_opt_util = (cf_reach_prob + counterfactual_reach_prob) * self.br.q_value(state, action)

      info_state_node.current_cfr_regret[action] += cfr_regret
      if self._iteration < 2:
        info_state_node.last_cfr_regret[action] += cfr_regret
      info_state_node.opt_util[action] += cur_opt_util
      info_state_node.cumulative_utility[action] += counterfactual_reach_prob * state_value_for_player
  ...
  def _clear_opt_regret(self):
    for info in self._info_state_nodes.values():
      info.opt_util.clear()
      info.current_cfr_regret.clear()
 
  def _update_current_policy_with_exploration(self, player):
    regret_heap = []
    _updated_info = set()
    updated_info_count = 0

    while updated_info_count < self._update_count:
      if len(regret_heap) > 0:
        _, sa_pair = heappop(regret_heap)
        state, action = sa_pair
        self._update_and_add_heap(state.child(action), regret_heap, _updated_info, player)
      else:
        state = self._game.new_initial_state()
        self._update_and_add_heap(state, regret_heap, _updated_info, player)
      updated_info_count += 1
    
    for info_state in self._info_state_nodes.keys():
      info_state_node = self._info_state_nodes[info_state]
      if info_state_node.player == player and info_state not in _updated_info:
        for action in info_state_node.legal_actions:
          info_state_node.cumulative_regret[action] += info_state_node.last_cfr_regret[action]

  def _update_and_add_heap(self, state, regret_heap, updated_info, player):
    if state.is_terminal():
      return
    if state.is_chance_node():
      outcomes, probs = zip(*state.chance_outcomes())
      outcome = np.random.choice(outcomes, p=probs)
      self._update_and_add_heap(state.child(outcome), regret_heap, updated_info, player)
      return
    ...
    if current_player != player:
      ...
      for a in legal_actions:
        sampling_policy.append(info_state_policy[a])
      sampled_action = np.random.choice(legal_actions, p=sampling_policy)
      self._update_and_add_heap(state.child(sampled_action), 
                                regret_heap, updated_info, player)
      return
    
    assert current_player == player
    if info_state not in updated_info:
      for action in state.legal_actions():
        info_state_node.cumulative_regret[action] += info_state_node.current_cfr_regret[action]
        info_state_node.last_cfr_regret[action] = info_state_node.current_cfr_regret[action]
      updated_info.add(info_state)

    for action in state.legal_actions():
      full_reg = info_state_node.opt_util[action] - info_state_node.cumulative_utility[action]
      full_reg += np.random.uniform(-1e-6, 1e-6)
      heappush(regret_heap, (-full_reg, (state, action)))
    return
  ...
  def evaluate_and_update_policy(self):
    ...
    if self._alternating_updates:
      for player in range(self._game.num_players()):
        ...
        self._update_current_policy_with_exploration(player)
...
class CFRSolver(_CFRSolver):
  ...
  def __init__(self, game, update_count):
